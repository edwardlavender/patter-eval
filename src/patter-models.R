...
#' Assemble (update) a list of filter arguments

assemble_args <- function(sim, args, acoustics, archival, direction, algorithm) {

  # Define direction
  args$.direction <- direction

  # (optional) Assemble acoustic containers
  # (optional) TO DO: define read_containers(sim, direction)
  containers <- assemble_acoustics_containers(acoustics,
                                              .direction = direction,
                                              .mobility = sim$mobility)

  # Collect observations & model_obs & update args
  if (algorithm == "acpf") {
  ...
  args$.yobs       <- yobs
  args$.model_obs  <- model_obs
  args$.n_particle <- np
  args$.n_resample <- as.numeric(np)
